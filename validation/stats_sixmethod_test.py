...
def repo_stats(filename, path):
    ...
    result_raw = result_raw.drop(columns=[5,8])
    print(result_raw)
    ...
    win_range = np.quantile(result_raw.iloc[:,1:7].to_numpy(), 0.424)
    ...
    result_win = [round(num/1628, 2) for num in result_win]
    ...
    with open("../result_stats/stats_six.csv", "a+") as output:
